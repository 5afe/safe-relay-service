...
class Command(BaseCommand):
    help = 'Deploys proxy factory using first unlocked account on the node if `ganache -d` is found and contract ' \
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('Checking if proxy factory was already deployed on %s' %
                                             proxy_factory_address))
        if ethereum_client.is_contract(proxy_factory_address):
            self.stdout.write(self.style.NOTICE('Proxy factory was already deployed on %s' % proxy_factory_address))
        else:
