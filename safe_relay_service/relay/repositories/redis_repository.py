...
class RedisRepository:
    ...
    def nonce_lock(self, ethereum_client: EthereumClient, address: str, lock_timeout: Optional[int] = None,
                   key_timeout: Optional[int] = None):
        return EthereumNonceLock(self.redis, ethereum_client, address, lock_timeout=lock_timeout,
...
class EthereumNonceLock:
    ...
    def __enter__(self):
        with self.redis.lock(self.redis_lock_key, timeout=self.timeout):
            ...
            if tx_nonce == 1:  # Empty cache will return 1 on INCR, so we need to make sure nonce is alright
