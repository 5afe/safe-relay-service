...
@app.shared_task()
def payment_status_task(retry: bool=True) -> None:
    lock = redis.lock("tasks:deploy_safes_task", timeout=LOCK_TIMEOUT)
    have_lock = lock.acquire(blocking=False)
    if not have_lock:
        logger.debug('exiting')
    ...
    try:
        logger.debug('Starting deploy safes task')
        pending_to_execute = SubscriptionTransactions.objects.pending_to_execute()
        #should have access to the safemultisig object and its safe
        logger.debug('%d safes pending to deploy', len(pending_to_execute))

        for sub_txn_execute in pending_to_execute:
            subscription_module_address = sub_txn_execute.safe.subscription_module_address
            txn_signed = sub_txn_execute.tx_signed
            ...
            if not safe_deployed_tx_hash:
                ...
                try:
                    ...
                    if creation_tx_hash:
                        ...
                        logger.info('Safe=%s creation tx has just been sent to the network with tx-hash=%s',
                                    safe_address,
                                    creation_tx_hash)
                        sub_txn_execute.safe_deployed_tx_hash = creation_tx_hash
                        sub_txn_execute.save()
                except TransactionAlreadyImported:
                    logger.warning("Safe=%s transaction was already imported by the node")
                    return
                except ValueError:
                    ...
                    logger.warning("Safe=%s was affected by reorg, let's check again receipt for tx-hash=%s",
                                   safe_address,
                                   sub_txn_execute.deployer_funded_tx_hash,
                                   exc_info=True)
                    sub_txn_execute.deployer_funded = False
                    sub_txn_execute.save()
                    check_deployer_funded_task.apply_async((safe_address,), {'retry': retry}, countdown=20)
            else:
                # Check if safe proxy deploy transaction has already been sent to the network
                ...
                if ethereum_service.check_tx_with_confirmations(safe_deployed_tx_hash,
                                                                settings.SAFE_FUNDING_CONFIRMATIONS):
                    logger.info('Safe=%s was deployed!', sub_txn_execute.safe.address)
                    sub_txn_execute.safe_deployed = True
                    sub_txn_execute.save()
                    # Send creation notification
                    send_create_notification.delay(safe_address, safe_creation.owners)
                elif (sub_txn_execute.modified + timedelta(minutes=10) < timezone.now()
                      and not ethereum_service.get_transaction_receipt(safe_deployed_tx_hash)):
                    # A reorg happened
                    logger.warning('Safe=%s deploy tx=%s was not found after 10 minutes. Trying deploying again...',
                                   sub_txn_execute.safe.address, safe_deployed_tx_hash)
                    sub_txn_execute.safe_deployed_tx_hash = None
                    sub_txn_execute.save()
    finally:
        if have_lock:
            lock.release()
