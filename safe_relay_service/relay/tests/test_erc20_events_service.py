...
class TestErc20EventsService(TestCase, SafeTestCaseMixin):
    ...
    def test_erc20_process_addresses(self):
        ...
        confirmations = 10
        erc20_events_service = Erc20EventsService(self.ethereum_client,
        ...
        to_tx_hash = self.send_tx(erc20_contract.functions.transfer(
            account_1.address, amount // 2).buildTransaction({'from': owner_account.address}), owner_account)
        self.send_tx(erc20_contract.functions.transfer(
            account_3.address, amount // 2).buildTransaction({'from': owner_account.address}), owner_account)
        # `account1` sends `amount // 2` (all) to `account_2`
        from_tx_hash = self.send_tx(erc20_contract.functions.transfer(
            account_2.address, amount // 2).buildTransaction({'from': account_1.address}), account_1)
        ...
        for erc20_event in EthereumEvent.objects.erc20_events():
            self.assertEqual(erc20_event.topic, self.ethereum_client.erc20.TRANSFER_TOPIC.hex())
